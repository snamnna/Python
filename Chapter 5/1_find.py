...
class Tree():
    ...
    def _find(self, data):
        ...
        # Niin kauan kun on nodeja
        while current_node:
            # Jos löydettiin se mitä haluttiin
            if data == current_node.data:
                return current_node

            # Jos verrattava on pienempi kuin root, lähdetään vasemmalle puolelle etsimään
            if data < current_node.data:
                current_node = current_node._left_child
            # Jos suurempi, lähdetään etsimään oikealta puolelta
            else:
                current_node = current_node._right_child
                
        # Palautetaan None jos aiemmassa while loopissa ei ole löydetty dataa
        return None