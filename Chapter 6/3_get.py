...
class HashTable():
    ...
    def get(self, key, alternative=None):
        ...
        """
        # Lasketaan ensin avaimelle hash arvo
        hash = self._hash(key)
        
        # Etsitään avain taulukosta hashin arvon perusteella
        searched = self._find_key(hash, key)
        
        # Jos avainta ei löydy, palautetaan alternative
        if searched is None:
            return alternative
        else:
        # Jos löytyy, palautetaan kyseisen avaimen arvo hash taulukosta
            return self.slots[searched].value