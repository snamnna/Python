...
class SinglyLinkedList():
    ...
    def remove(self, index):
        ...
        # Jos index on pienempi kuin nolla tai suurempi kuin listan koko, error
        if index < 0 or index >= self._size:
            raise ValueError('Index out of bounds')
        ...
        # Jos index on 0, poistetaan ensimmäinen node
        if index == 0:
            value = self._head.data
            # Vaihdetaan seuraava node headiksi
            self._head = self._head.next
            # Jos listan koko on 1, ei ole tailia
            if self._size == 1:
                self._tail = None
        # Jos poistetaan mikä tahansa muu solmu kuin ensimmäinen
        else:
            previous_node = self._head
            # Loopataan läpi jotta löydetään indexiä edeltävä solmu
            for i in range(index - 1):
                # Muutetaan previous nodeksi seuraava node
                previous_node = previous_node.next
            value = previous_node.next.data
            
            # Jos index vastaa viimeistä nodea, muutetaan indexiä edeltävä node tailiksi ja se osoittamaan tyhjään
            if index == self._size - 1:
                self._tail = previous_node
                previous_node.next = None
            # Muutoin laitetaan seuraavaksi nodeksi poistettavaa nodea seuraava node
            else:
