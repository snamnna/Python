...
class DoublyLinkedList:
    ...
    def insert(self, index, value):
        ...
        Returns: None
        """
        # Jos index on pienempi kuin nolla tai isompi kuin listan koko
        if index < 0 or index > self._size:
            raise ValueError('Index out of bounds')
        
        # Jos lisätään listan alkuun
        if index == 0:
            new_node = ListNode(value, next=self._head)
            if self._head is not None:
                self._head.prev = new_node
            self._head = new_node
        # Etsitään lisättävää nodea edeltävä
        else:
            current_node = self._head
            for _ in range(index - 1):
                current_node = current_node.next

            # Luodaan uusi lisättävä node ja laitetaan pointerit sille
            new_node = ListNode(value, next=current_node.next, prev=current_node)
            if current_node.next is not None:
                current_node.next.prev = new_node
            current_node.next = new_node
        
        # Jos lisätään loppuun, voidaan päivittää suoraan tail
        if index == self._size:
            self._tail = new_node
